<template>
  <container class="step4">
    <group title="预览您即将提交的信息(如需修改信息请联系客服，电话189388318838)" label-width="7em" label-margin-right="0.5em" label-align="center">
      <cell title="我的项目" value-align="right" link="/accountant/step1">
        <span v-show="services.length === 0">编辑</span>
        <div v-show="services.length > 0">
          <span v-for="(item, i) in services" v-html="item + ' '"></span>
        </div>
      </cell>
      <cell title="我的服务的城市" value-align="right" link="/accountant/step2">
        <span v-show="serveCitys.length === 0">编辑</span>
        <div v-show="serveCitys.length > 0">
          <span v-for="(item, i) in serveCitys" v-html="item + ' '"></span>
        </div>
      </cell>
      <cell title="我的姓名" :value="realname" value-align="right" link="/accountant/step3"></cell>
      <cell title="我的手机" :value="phone" value-align="right" link="/accountant/step3"></cell>
      <cell title="我的地址" :value="address" value-align="right" link="/accountant/step3"></cell>
      <cell title="我的证件照片" value-align="right" link="/accountant/step3">
      ...
      <cell title="我的从业经历" value-align="right" link="/accountant/step3">
      ...
      <cell title="登陆密码" value="******" value-align="right" link="/accountant/step3"></cell>
    </group>
      ...
      async onSure () {
        ...
        if (res.code === 20000) {
          ...
          this.$vux.alert.show({
            ...
            onHide () {
              self.$router.push('/accountant/login?path=order')
            }
          })
        } else {
          this.$vux.alert.show({
